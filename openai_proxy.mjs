...
function buildChatPrompt(input) {
  return [
    ...
    "当 hasMarketContext 为 false 时，说明最新问题没有关联到股票行情：必须直接回答最新问题本身，禁止从历史或自选股中挑一只股票继续分析，也不要输出支撑、压力、涨跌或条件观察。",
    ...
    `hasMarketContext：${Boolean(input.hasMarketContext)}\n`,
    `最近对话：${input.history || "无"}\n`,
    `用户问题：${input.question || ""}\n`,
    "只有股票研究问题需要解释依据和风险；问候和普通对话只需紧扣最新问题自然作答，不要硬加投资提示。",
  ].join("");
}
