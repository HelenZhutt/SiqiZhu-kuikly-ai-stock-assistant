package com.example.stockaidemo.adapter

import com.example.stockaidemo.KRApplication
...
class KRImageAdapter(val context: Context) : IKRImageAdapter {
    ...
    private fun requestImage(
        ...
        // asDrawable 同时支持静态图片和 GifDrawable，避免不安全的泛型强转。
        val requestBuilder = Glide.with(KRApplication.application)
            .asDrawable()
            .load(src)
