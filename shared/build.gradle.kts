...
android {
    namespace = "com.example.stockaidemo.shared"
    compileSdk = 34
    defaultConfig {
        minSdk = 21
    }
...
configure<KuiklyConfig> {
    // JS 产物配置
    js {
        // 构建产物名，与 KMM 插件 webpackTask#mainOutputFileName 一致
...
// Android-only development should still sync on Macs that do not have CocoaPods.
// Once CocoaPods is installed in a standard location, the iOS pod task remains enabled.
val cocoaPodsAvailable = listOf(
    "/opt/homebrew/bin/pod",
    "/usr/local/bin/pod",
    "/usr/bin/pod"
).any { file(it).isFile }

if (!cocoaPodsAvailable) {
