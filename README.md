...
2. **多入口股票搜索**：中文名不用打全称，拼音、首拼、A股/港股代码都能搜（如 `茅台`、`payh`、`000001`）。港股上市股份后缀会自动补全，`YSKJ` 也能命中 `YSKJW`。先出本地匹配，再补全市场结果，并过滤基金和指数，只保留能进详情的股票；重名时在问答里让用户点选，不会猜错标的。
3. **真实 AI 接入**：Groq Compound（或可切换的 Gemini/OpenAI）密钥只保存在本机代理进程，不写入 App 或仓库；代理强制返回结构化 JSON，页面展示稳定可控。
4. **可靠演示**：网络、额度或 API 异常时自动切换本地知识库，界面会明确显示“离线兜底”，不会无限加载或伪装在线结果。
5. **可验证的行情图表**：代理同时获取当前报价与 20 个交易日复权收盘价，跨端 Canvas 按真实数据绘制折线。
6. **双股量化比较**：涨跌幅、日内振幅、价格位置与风险等级并排呈现，不用绝对成交量误判强弱。
7. **可解释输出**：回答与卡片明确显示数据来源、行情时间、依据字段和教学免责声明。
8. **工程化分层**：页面、组件、数据模型、行情仓库、AI 服务和本机代理分离；`StockChatPage` 已拆出 MarkdownRenderer、ChatBubble 与 StockInsightCard。
9. **连续对话体验**：会话保存在设备本地；新对话不会删除旧记录，历史侧栏按最近打开时间排序，并支持删除单条会话。
10. **明确错误恢复**：行情刷新失败可再次点击刷新；详情和问答在线调用失败会显示橙色重试入口，并保留可用的本地结果。
11. **跨端一致**：Android 与 iOS 共用 Kotlin 业务/UI；代理地址自动适配 Android Emulator 的 `10.0.2.2` 与 iOS Simulator 的 `localhost`。
12. **动态股票发现**：搜索命中后按市场区分展示，选中即拉取真实行情与 20 日走势、加入本地自选并进入详情；重启 App 后恢复动态自选。
13. **AI 与图表联动**：详情页可切换 5 日/20 日窗口；横轴分别显示 3/4 个稀疏日期刻度，支撑位和压力位的数值直接标在线上，并展示置信度进度。
14. **可解释关键价位**：支撑/压力由最近 20 日收盘序列的局部低点/高点确定（无有效局部极值时退回窗口极值），作为结构化上下文交给 AI，而不是把当日 high/low 包装成模型判断。
15. **对话按需取数**：AI 对话识别明确的 A/H 股名称或代码；本地尚无该股票时临时查询真实报价与 20 日历史，再生成个股或比较卡片。临时查询不会自动加入自选，收藏仍由用户主动决定。
16. **无套娃导航**：对话中的任意股票卡片都可进入对应详情；从详情返回时回到原 AI 对话，不为每只股票继续叠加新的聊天页。
17. **首页行情密度**：每只自选股使用真实历史收盘绘制 mini sparkline，在进入详情前即可快速识别方向。
18. **数据驱动的 AI 市场雷达**：根据当前自选的涨跌、领涨/回调股票和平均振幅生成一句可验证的市场洞察，并以“查看今日解读”进入完整分析。
19. **Generative UI 输出**：单股问题、对比问题和风险排行分别呈现不同结构化组件，风险排行还可逐项进入对应详情。
20. **金融语义配色**：红涨绿跌仅用于行情；AI 判断改用“偏强/偏弱/关注”的蓝色语义，风险单独使用青绿/橙/红等级色。
...
环境：JDK 17（Android Studio 自带即可）。

不启动代理也能完整演示：首页 4 只股票已带 20 日走势，详情图表、AI 卡片和问答都会显示，并标明「离线兜底」。下面的 Key 只在希望看**联网真实行情 + 在线模型**时需要。

### 1. （可选）获取 Groq API Key 并启动代理

这里是 **Groq**（[console.groq.com](https://console.groq.com)），不是 xAI 的 Grok。**API Key 免费申请，个人试用不用绑信用卡。** 本机还需 [Node.js 18+](https://nodejs.org/)。若打不开 Groq 网站或 `api.groq.com`，跳过本步即可，App 会自动走本地演示数据。

1. 打开 [https://console.groq.com](https://console.groq.com)，用 Google 或邮箱注册/登录。
2. 进入 [API Keys](https://console.groq.com/keys)，点 **Create API Key**，名称随意（例如 `zhishi-demo`）。
3. **立刻复制**生成的密钥（以 `gsk_` 开头）。页面只显示一次，关掉就看不到了。
4. 在项目根目录执行：
...
5. 终端提示粘贴 Key 时，直接粘贴后回车。输入过程不显示，**也不会写入项目文件**，不要把 Key 提交到 Git。
6. 看到下面这几行即表示联网代理已就绪：
...
然后保持这个终端开着，再去 Android Studio 运行 App。首页应显示「真实行情」，详情和问答会走在线模型；失败时仍会降级到本地卡片，不会卡死。
...
1. Android Studio 顶部设备栏选择任意已启动的模拟器（Pixel 等即可）。
2. 运行配置选择 `androidApp`。
3. 点击绿色运行按钮。
...
Android 模拟器不能直接使用 macOS 的中文输入法组合文字，需要使用模拟器内的 Gboard。聚焦输入框后点击键盘浮动栏底部的 `US/拼` 即可切换。新建模拟器时可按以下路径添加：
...
鸿蒙工程位于 `ohosApp`，与 Android / iOS 共用同一套 Kuikly 页面，首屏同样启动 `StockList`。鸿蒙使用独立的 `settings.ohos.gradle.kts` 和 `shared/build.ohos.gradle.kts`，不会替换 Android/iOS 的构建配置。

> **关于演示视频：** 鸿蒙端已经实现，但本次提交的视频里没有 HarmonyOS 录屏。作者当前在美国，鸿蒙开发与模拟器环境主要面向中国大陆，本地无法稳定跑起官方模拟器或真机，因此只提交了可运行的 `ohosApp` 代码，没有鸿蒙演示视频。若评委在中国大陆，用 DevEco Studio 打开 `ohosApp` 即可验证与 Android/iOS 相同的业务页面。
...
1. 不启代理时：点腾讯控股，展示 20 日折线、支撑/压力和离线 AI 卡片。若已启动 Groq 代理：搜索“招商银行”，展示名称/首拼/代码搜索、真实行情和加入自选。
2. 点腾讯控股，展示行情、AI 解读、风险和关键价位。
3. 点“继续追问 AI”，展示从详情到问答的上下文承接。
4. 再问“对比平安银行和贵州茅台”，展示双股量化卡片和各自真实指标。
5. 点击回答中的股票卡片返回详情，完成双向页面闭环。
6. 简短说明断网时自动降级，强调演示可靠性与密钥安全。

> 初始列表保留演示快照以保证离线可演示；点击“刷新真实行情”后会替换为代理获取的当前行情。该免密行情源适合比赛 Demo，不承诺生产 SLA；正式产品应更换为有授权和服务保证的行情供应商。AI 内容只用于教学展示，不构成投资建议。
